/**
 * Combined Notifications Panel v8.10.5
...
try {
  ...
  css  = LitElement.prototype?.css  || window.css  || ((strings, ...values) => {
    let cssText = strings[0];
    values.forEach((v, i) => { cssText += (v || '') + strings[i + 1]; });
...
if (typeof css  !== "function") css  = (strings, ...values) => {
  let cssText = strings[0];
  values.forEach((v, i) => { cssText += (v || '') + strings[i + 1]; });
  return { cssText, _$cssResult$: true };
};
...
try {
  console.log('%cCombined Notifications v8.10.2 → Starting definePanel()', 'color:#39FF14; font-weight:bold');
  definePanel();
  console.log('%cCombined Notifications v8.10.2 → Successfully registered', 'color:#39FF14; font-weight:bold');
} catch (e) {
  ...
  const errorHTML = `
    <div style="position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:#1e2535;color:#fc8181;padding:30px 40px;border-radius:16px;border:3px solid #fc8181;z-index:999999;font-family:sans-serif;max-width:90%;text-align:center;box-shadow:0 20px 60px rgba(0,0,0,0.8);">
      ...
      <p style="margin:8px 0">Version 8.10.2</p>
...
const CN_STYLES = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .keyword-count { font-size: 0.78rem; color: #63b3ed; font-family: monospace; font-weight: 600; margin-top: 4px; display: block; }
  .entity-list-title { font-size: 0.78rem; color: #63b3ed; font-family: monospace; font-weight: 600; }
  ...
  .and-title { padding: 8px 12px; font-size: 0.9rem; font-weight: 700; color: #63b3ed; font-family: 'DM Sans', sans-serif; }
  ...
  .entity-list { border: 1px solid rgba(255,255,255,0.08); border-radius: 8px; overflow: hidden; }
  .entity-list-header {
    display: flex;
    justify-content: space-between;
    align-items: center;
  ...
  .empty-hint { font-size: 0.85rem; color: #64748b; font-style: italic; padding: 4px 0; }
  .dialog-footer {
    display: flex;
    align-items: center;
  ...
  .domain-chip.included { background: rgba(47,207,118,0.1); border-color: rgba(47,207,118,0.3); color: rgb(47,207,118); }
  .domain-chip.excluded { background: rgba(246,173,85,0.08); border-color: rgba(246,173,85,0.2); color: #f6ad55; }
  @keyframes chip-pulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.4; } }
  ...
  .entity-picker-empty { padding: 12px; color: #64748b; font-size: 0.85rem; font-family: 'DM Sans', sans-serif; text-align: center; }
  .entity-picker-item {
    display: flex; align-items: center;
    justify-content: space-between;
  ...
  .entity-picker-item-left { display: flex; align-items: center; gap: 8px; min-width: 0; flex: 1; }
  ...
  .entity-picker-friendly { font-size: 0.85rem; color: #e2e8f0; font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .entity-picker-id { font-size: 0.7rem; color: #64748b; font-family: monospace; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .entity-picker-state {
  ...
  .entity-label-input:focus { border-color: rgba(99,179,237,0.4); color: #e2e8f0; }
  ...
  .overview-key-item { display: flex; align-items: center; gap: 6px; }
  ...
  .overview-scroll-wrap { flex: 1; overflow-y: auto; min-height: 0; border-radius: 12px; }
  ...
  .overview-container > .overview-domain-divider:first-of-type { border-top: none; }
  .overview-thead {
    display: grid;
...
class CombinedNotificationsPanel extends LitElement {
  ...
  _injectStyles() {
    if (!this.shadowRoot || this.shadowRoot.querySelector('style[data-cn]')) return;
    const style = document.createElement('style');
    style.setAttribute('data-cn', '1');
    style.textContent = CN_STYLES;
        ...
        <div class="panel-root">
          <div class="dialog">
            ${this._error
              ...
              : html`<div class="loading" style="min-height:100vh;">Loading configuration...</div>`}
          </div>
        </div>
      `;
    }
      ...
      <div class="panel-root">
        <div class="dialog">
          ...
          <div class="dialog-footer">
            <span class="version-stamp">pja v8.10.2</span>
        ...
        <div class="group-body">
          ...
          <div class="icon-row">
            <div class="icon-field">
              ...
              ` : html`
                <input type="text" .value="${c.icon_all_clear || "mdi:hand-okay"}"
                  placeholder="mdi:hand-okay"
                  @input="${e => this._set("icon_all_clear", e.target.value)}">
                <div class="hint">mdi:hand-okay, mdi:shield-check, etc.</div>
              `}
            </div>
            <div class="icon-field">
              ...
              ` : html`
                <input type="text" .value="${c.icon_alert || "mdi:alert-circle"}"
                  placeholder="mdi:alert-circle"
                  @input="${e => this._set("icon_alert", e.target.value)}">
                <div class="hint">mdi:alert, mdi:exclamation-thick, etc.</div>
              `}
            </div>
          </div>
        </div>
          ...
          <div class="cond-body">
            ...
            ${condition.use_label_template ? html`
              <div class="field">
                ...
                <textarea
                  rows="4"
                  style="width:100%;padding:9px 12px;background:#0d0f18;border:1px solid rgba(255,255,255,0.1);border-radius:8px;color:#e2e8f0;font-family:monospace;font-size:0.85rem;outline:none;resize:vertical;line-height:1.5;"
                  placeholder="${'e.g. multi-line:\n{% set d = states(\'sensor.lightning_distance\') %}\nLightning {{ d }} miles away'}"
                  @input="${e => this._setCondition(index, "label_template", e.target.value)}"
                  .value="${condition.label_template || ""}"></textarea>
                <div class="hint"><em>Use {{ states('sensor.entity_id') }} to insert live values. Full Jinja2 supported — multiple lines allowed. Example: Lightning {{ states('sensor.lightning_distance') }} miles away</em></div>
              </div>
              <div class="field">
